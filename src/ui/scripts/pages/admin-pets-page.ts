...
class AdminPetsPage {
  ...
  private cacheElements() {
    ...
    this.clearSearchBtn = document.getElementById('clearSearch') as HTMLButtonElement
    this.userFilter = document.getElementById('userFilter') as HTMLSelectElement
    this.tableBody = document.getElementById('petsTableBody') as HTMLTableSectionElement
  ...
  private bindEvents() {
    ...
    document.getElementById('editPetForm')?.addEventListener('submit', (e) => this.handleEditSubmit(e))
  ...
  private getSpeciesClass(species: string): string {
    ...
    if (s.includes('cachorro') || s.includes('cão') || s.includes('dog')) return 'cachorro'
    if (s.includes('gato') || s.includes('cat')) return 'gato'
    if (s.includes('ave') || s.includes('pássaro') || s.includes('bird')) return 'ave'
  ...
  private getSpeciesIcon(species: string): string {
    const s = species.toLowerCase()
    if (s.includes('cachorro') || s.includes('cão') || s.includes('dog')) return 'dog'
    if (s.includes('gato') || s.includes('cat')) return 'cat'
    if (s.includes('ave') || s.includes('pássaro') || s.includes('bird')) return 'bird'
  ...
  private openEditModal(petId: string) {
    ...
    // Populate form
    const nameInput = document.getElementById('editPetName') as HTMLInputElement
    const speciesInput = document.getElementById('editPetSpecies') as HTMLInputElement
    ...
    if (speciesInput) speciesInput.value = pet.species
